<template>
  <section id="contact" class="py-24 relative overflow-hidden">
    <!-- Background Glow -->
    <div class="absolute top-1/2 left-0 w-[600px] h-[600px] bg-primary-500/5 rounded-full blur-3xl -translate-y-1/2 -translate-x-1/4 pointer-events-none"></div>
    
    <div class="container mx-auto px-6 relative z-10">
      <div class="mb-16 max-w-2xl mx-auto text-center" ref="headerRef" :class="getAnimClass(headerAnim)">
        ...
        <p class="text-slate-400 text-lg">Have a project in mind or want to discuss opportunities? I'd love to hear from you.</p>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
        <!-- Form Side -->
        <div ref="formRef" :class="getAnimClass(formAnim)" class="glass-card p-8 md:p-10 rounded-3xl border border-white/5 bg-white/5 backdrop-blur-md">
          <form @submit.prevent="submitForm" class="space-y-6">
            <div class="space-y-2">
              <label for="name" class="block text-sm font-medium text-slate-300">Name</label>
              <UInput id="name" v-model="form.name" icon="i-lucide-user" placeholder="Your name" size="lg" :ui="{ base: 'bg-white/5 border-white/10 text-white placeholder-slate-500 focus:border-primary-500' }" required />
            </div>
            
            <div class="space-y-2">
              <label for="email" class="block text-sm font-medium text-slate-300">Email</label>
              <UInput id="email" v-model="form.email" type="email" icon="i-lucide-mail" placeholder="your@email.com" size="lg" :ui="{ base: 'bg-white/5 border-white/10 text-white placeholder-slate-500 focus:border-primary-500' }" required />
            </div>

            <div class="space-y-2">
              <label for="message" class="block text-sm font-medium text-slate-300">Message</label>
              <UTextarea id="message" v-model="form.message" placeholder="Tell me about your project..." :rows="5" size="lg" :ui="{ base: 'bg-white/5 border-white/10 text-white placeholder-slate-500 focus:border-primary-500' }" required />
            </div>

            <UButton type="submit" label="Send Message" icon="i-lucide-send" color="primary" size="lg" block class="mt-8 py-3 font-semibold shadow-lg shadow-primary-500/20 hover:shadow-primary-500/40 transition-shadow" />
          </form>
        </div>

        <!-- Contact Info Side -->
        <div ref="infoRef" :class="getAnimClass(infoAnim)" class="flex flex-col gap-6 justify-center">
          <a v-for="(info, index) in contactInfo" :key="index" :href="info.link" target="_blank" rel="noopener noreferrer" 
            ...
            <div class="w-14 h-14 rounded-2xl bg-primary-500/10 flex items-center justify-center group-hover:bg-primary-500/20 group-hover:scale-110 transition-all duration-300 shadow-inner shadow-primary-500/20">
              <UIcon :name="info.icon" class="w-6 h-6 text-primary-400 group-hover:text-primary-300" />
            </div>
            <div>
